...
class FitFunc_Client(jm.JobManager_Client):
    def __init__(self):
        super(FitFunc_Client, self).__init__(
            server="localhost",
    ...
    def func(args, const_args):
        ...
        res = minimize(fun=f_min, x0=np.array(args), method="BFGS")
...
class FitFunc_Server(jm.JobManager_Server):
    def __init__(self, const_args, num_samples, n, g_max, w_max):
        ...
        # init ancestor class
        super(FitFunc_Server, self).__init__(
            authkey=authkey,
        ...
        for i in range(num_samples):
            ...
            x0 = (
                tuple(g_re) + tuple(g_im) + tuple(w)
            )  # need tuple instead if numpy ndarray
    ...
    def process_new_result(self, arg, result):
        x, fun = result
        g, w, n = x_to_g_w(x)
        if (self.final_result == None) or (self.final_result[0] > fun):
