...
warnings.filterwarnings(
    "ignore", module="traitlets", append=False, category=DeprecationWarning
)
...
def test_Signal_to_terminate_process_list():
    ...
    def mother_proc():
        try:
            ...
            Signal_to_terminate_process_list(
                process_list=p,
                identifier_list=["proc_{}".format(i + 1) for i in range(n)],
            )
...
def start_server(
    n, read_old_state=False, client_sleep=0.1, hide_progress=False, job_q_on_disk=False
):
    print("START SERVER")
    args = range(1, n)
    with jobmanager.JobManager_Server(
        authkey=AUTHKEY,
        port=PORT,
        msg_interval=1,
...
def start_client(hide_progress=True):
    print("START CLIENT")
    jm_client = jobmanager.JobManager_Client(
        server=SERVER,
        authkey=AUTHKEY,
        port=PORT,
        nproc=3,
...
def test_start_server_with_no_args():
    ...
    with jobmanager.JobManager_Server(
        authkey=AUTHKEY, port=PORT, msg_interval=1, fname_dump="jobmanager.dump"
    ) as jm_server:
        jm_server.start()
...
def test_jobmanager_static_client_call():
    ...
    PORT += 1
    jm_client = jobmanager.JobManager_Client(
        server=SERVER, authkey=AUTHKEY, port=PORT, nproc=3, reconnect_tries=0
    )
    jm_client.func(arg=1, const_arg=1)


@pytest.mark.skipif(sys.version_info.major == 2, reason="causes unknown trouble")
def test_client():
    global PORT
    PORT += 1
    p_server = None
    ...
    try:
        ...
        jmc = jobmanager.JobManager_Client(
            server=SERVER, authkey=AUTHKEY, port=PORT, nproc=3, reconnect_tries=0
        )
        jmc.start()
...
def test_jobmanager_basic():
    ...
    for jqd in [False, True]:
        ...
        try:
            # start a server
            p_server = mp.Process(
                target=start_server, args=(n, False), kwargs={"job_q_on_disk": jqd}
            )
            p_server.start()
            time.sleep(0.5)
...
def test_shutdown_server_while_client_running():
    ...
    for signame, sig in sigs:
        ...
        try:
            ...
            print(
                "[+] server and client joined {}".format(
            ...
            ac = data["job_q"]
            args_set = {
                binfootprint.dump(ac.data["_" + str(id_)]) for id_ in ac._not_gotten_ids
            }
...
def test_client_status():
    ...
    try:
        ...
        client = Client_With_Status(server=SERVER, authkey=AUTHKEY, port=PORT, nproc=4)
        client.start()
        p_server.join(5)
        assert not p_server.is_alive()
    except:
...
def test_jobmanager_local():
    ...
    num_client = 4
    t0 = time.time()
    with jobmanager.JobManager_Local(
        client_class=jobmanager.JobManager_Client,
        authkey=AUTHKEY,
        port=PORT,
        const_arg=client_sleep,
        nproc=num_client,
...
def test_start_server_on_used_port():
    ...
    def start_server():
        ...
        arg = [10, 20, 30]
        with jobmanager.JobManager_Server(
            authkey=AUTHKEY, port=PORT, const_arg=const_arg, fname_dump=None
        ) as server:
...
def test_shared_const_arg():
    ...
    def start_client():
        ...
        client = myClient(server=SERVER, authkey=AUTHKEY, port=PORT, nproc=1)
...
def test_digest_rejected():
    ...
    client = Client_With_Status(
        server=SERVER, authkey=AUTHKEY + " not the same", port=PORT, nproc=4
    )
    try:
        client.start()
    except ConnectionError as e:
        print(
            "Not an error: caught '{}' with message '{}'".format(
